...
| 状态管理 | 纯 TS SceneManager | Vue 不直接管理流程图业务状态，只订阅核心状态快照 |
...
- 内核通过事件订阅或状态快照通知 UI 更新，避免把 Vue 响应式系统耦合进编辑器模型。
...
- `FlowDocument` 用于保存和加载，不保存运行时选中态、hover 态和交互态。
- `SceneSnapshot` 用于 UI 展示，是运行时状态的只读快照。
...
状态由纯 TS `SceneManager` 管理，Vue 不直接持有流程图业务数组。
...
export class SceneManager {
  ...
  createEdge(source: Endpoint, target: Endpoint) {
    // 校验端口规则后创建连线
  ...
  load(document: FlowDocument, registry: ElementRegistry) {
    this.rootBox = registry.createBox(document.root)
    this.edgeLayer = registry.createEdgeLayer(document.edges)
    this.emitChange()
  }
...
- Vue 通过 `subscribe` 获取 `SceneSnapshot`，不直接修改 class 内部状态。
- 属性面板调用 `updateNodeProps`、`updateEdgeProps` 这类命令，不直接改节点实例。
- 保存和加载只走 `toDocument()` 和 `load()`。
...
export function useFlowEditorCore() {
  ...
  function mount(options: {
    ...
    return core.value.scene.subscribe(nextSnapshot => {
...
这里 Vue 只追踪 `snapshot` 引用变化，不追踪节点 class 的内部字段。
...
| 事件 | 处理逻辑 |
| --- | --- |
| `pointerdown` | 命中端口则进入连线状态，命中节点则选中并进入拖动状态，命中空白则清空选中 |
| `pointermove` | 拖动节点时更新节点位置，连线状态下更新预览线终点 |
| `pointerup` | 拖动节点时结束拖动，连线状态下检查目标端口并创建连线 |
...
export const elementTemplates: ElementTemplate[] = [
  {
    ...
    ports: [
      { id: 'out', label: '输出', direction: 'output', offset: { x: 120, y: 24 } },
    ],
  },
  {
    type: 'task',
...
节点命中和端口命中委托给对应 `BaseNodeView`，这样菱形、圆形、特殊端口布局都能和绘制逻辑保持一致。连线命中可以先用“点到线段距离”近似实现。若后续使用贝塞尔曲线，再增加曲线采样命中或交给 `BaseEdgeView` 实现。
...
```text
SceneManager snapshot -> Vue shallowRef -> PropertyPanel props
...
FlowEditor
  ├─ useFlowEditorCore
  │   └─ 持有 FlowEditorCore 实例和 SceneSnapshot
  ├─ ElementPalette
  │   └─ 输出拖拽模板 type
  ├─ FlowCanvas
  ...
  └─ PropertyPanel
      ├─ 读取 SceneSnapshot 中的 selectedNode / selectedBox / selectedEdge
