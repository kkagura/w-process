...
迁移 workspace 后必须重新生成并提交 `pnpm-lock.yaml`，使 importer 包含根目录、`apps/web`、`packages/flow-core` 和 `packages/flow-node-renderer`。
...
8. 创建 `packages/flow-node-renderer`，配置 Node ESM 的 tsdown 构建并保持原生 Canvas external，先用固定场景 JSON 生成 PNG。
9. 增加导出模式、字体注册、尺寸安全限制和图层合成。
10. 增加核心单元测试、Node 渲染冒烟测试和跨运行时图片快照测试。
