...
- 编辑模式下，`NodeDrawContext.showPorts` 由当前节点的 hover、selected 和 connecting 状态派生；普通命中只检测可见端口，连线模式检测全部端口，避免隐藏端口产生不可见交互。
- Preview 模式默认隐藏端口；显式设置 `showPorts` 时绘制全部端口，不依赖编辑器交互态。
...
核心渲染器区分编辑模式和预览模式。`preview` 表示面向最终图片结果的纯内容预览，不使用 `export` 作为模式名称；文件编码和写入仍由 Node renderer 的导出 API 负责：
...
预览模式默认隐藏端口、选框、旋转柄、吸附线和其他编辑器操作反馈，并忽略 Scene 中残留的 selected、hovered、dragging 等交互状态。任何交互态都不得进入 `FlowDocument`。
...
Node renderer 同时提供编译后执行的 CLI，命令名为 `w-process-render`：

```bash
w-process-render ./scene.json
w-process-render ./scene.json --output ./scene.png
w-process-render ./scene.json --padding 40 --pixel-ratio 2 --background transparent
```

首版 CLI 约束：

- 只接受一个本地 `FlowDocument` JSON 文件，只输出 PNG。
- `--output` 省略时输出到输入文件同目录，文件名为 `<原文件名>.png`。
- 默认使用 `preview` 模式，隐藏网格、端口和交互覆盖层；`--show-grid`、`--show-ports` 可显式开启前两项。
- 支持重复传入 `--font <path>` 注册本地字体；字体文件不进入 `flow-core`。
- 输出文件已存在时默认拒绝覆盖，只有显式传入 `--force` 才允许替换。
- CLI 负责路径解析、文件读写、终端提示与退出码；`renderFlowImage()` 不产生文件系统副作用。
- 参数错误、文件不存在、JSON 无效、场景结构无效、未知元素类型和尺寸超限必须返回非零退出码，且不得留下半写入的目标文件。
- 正式命令执行 tsdown 生成的 `dist/cli.mjs`，不直接把 `.ts` 文件作为交付入口。

首版默认限制为画布单边不超过 `16384` 个物理像素，总物理像素不超过 `64_000_000`；限制在分配原生 Canvas 前完成检查。
...
- 首版固定字体使用 OFL-1.1 授权的 `@expo-google-fonts/noto-sans-sc` 完整 TTF，注册 400、600、700 三个字重；`flow-core` 默认字体族以 `Noto Sans SC` 为首选并保留系统字体回退。
...
`@napi-rs/canvas` 和 `@expo-google-fonts/noto-sans-sc` 必须声明在 `flow-node-renderer` 的 `dependencies` 中。tsdown 默认会将 `dependencies`、`peerDependencies` 和 `optionalDependencies` 外置；仍然显式配置 `neverBundle`，用于强调原生模块绝不能被打进 JavaScript bundle。`@w-process/flow-core` 使用 `workspace:*` 声明为普通依赖，默认保持为运行时包依赖。
...
因此，Node 原生 TypeScript 只可用于仓库内部的小型开发脚本或测试辅助脚本；`flow-core` 的正式入口指向 tsdown 生成的 `dist/*.js`，`flow-node-renderer` 指向 `dist/*.mjs`，并同时提供对应声明文件。

相关代码应坚持使用 `import type`、ESM 和显式公共入口。是否采用 `.ts` 扩展导入由最终 tsdown、TypeScript 与 NodeNext 配置统一验证，不能在包内混用多套导入约定。

#### npm 公开发布规则

`@w-process/flow-core` 和 `@w-process/flow-node-renderer` 作为公开 scoped npm 包发布，根 workspace 与 `@w-process/web` 继续保持 `private: true`。公开包必须声明 `publishConfig.access = "public"` 和 npm 官方 registry，避免本机私有镜像配置改变发布目标。

发布约束：

- Node renderer 对核心包使用 `workspace:^`，打包和发布时转换为对应的公开 semver 范围。
- 两个公开包都必须提供 `prepack`，在生成 tarball 或发布前执行类型检查和构建。
- `files` 只包含运行时所需的 `dist`；npm 自动包含的 `package.json` 和包内 README 用于元数据与使用说明。
- 包元数据必须包含描述、关键字、仓库地址、主页和问题反馈地址；许可证由项目所有者明确选择，智能体不得擅自添加。
- 发布顺序固定为 `flow-core` 后 `flow-node-renderer`，避免 renderer 的公开依赖尚不可解析。
- 正式发布前必须分别执行 `pnpm pack`，检查 tarball 中的文件和依赖版本，并在仓库外的隔离临时目录安装 tarball、运行 CLI 冒烟测试。
- scoped 包发布时显式使用 `--access public` 和 `--registry https://registry.npmjs.org/`。
- 仓库根目录必须提供不含认证信息的 `.npmrc`，将默认 registry 和 `@w-process` scope registry 都固定为 `https://registry.npmjs.org/`，覆盖用户目录中可能存在的私有仓库配置。
- npm 登录、组织 scope 权限和双因素认证由发布者账号控制；没有有效登录态或 `@w-process` scope 权限时不得绕过认证。
...
1. 创建 `packages/flow-node-renderer`，配置 Node ESM 的 tsdown 构建并保持原生 Canvas external，实现 `w-process-render <scene.json>` 生成 PNG。
2. 增加预览模式、字体注册、尺寸安全限制和图层合成。
...
项目内维护 `skills/w-process-diagram`，用于指导支持 Skill 的智能体把“使用 w-process 画一张流程图、泳道图或架构图”等自然语言需求转换为 `FlowDocument`，再调用已安装的 `w-process-render` CLI 生成 PNG。Skill 是智能体编排和领域知识层，不属于 pnpm workspace，也不得复制渲染内核或引入另一套 Canvas 实现。

Skill 的职责边界：

- `SKILL.md` 只保存触发描述、核心工作流、失败处理和交付规则。
- `references/` 保存当前 `FlowDocument` 结构、已注册元素目录、端口约束和布局规则；这些内容必须以 `flow-core` 的 DTO、模板和注册表为依据。
- `assets/templates/` 保存可复制修改的合法场景 JSON，至少覆盖基础流程、审批分支、泳道和架构分层。
- 智能体先生成场景 JSON，再调用 CLI；CLI 继续作为运行时文档校验、字体注册、尺寸限制和图片编码的唯一实现。
- Skill 不负责安装 npm 包。找不到 `w-process-render` 时应停止渲染并提示用户手动安装或提供命令路径。
- 渲染完成后应查看生成图片，检查文字截断、元素重叠、容器越界和连线表达；需要时修改 JSON 后重新渲染。
- 中间 JSON 和最终 PNG 写入用户当前工作区或用户指定目录，不写入 Skill 自身目录，不擅自覆盖现有文件。

Skill 源码随项目进行版本管理，但具体复制或安装到哪一种智能体的 Skills 目录由使用者手动处理。后续如果需要面向多个智能体客户端提供稳定的工具协议，可以单独增加 MCP Server；MCP 只暴露 schema 查询、场景校验和渲染等确定性能力，自然语言理解仍由 Skill 或调用方智能体负责。
